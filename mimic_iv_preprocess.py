...
class LabTOPPreprocessor:
    def __init__(
        ...
        if self.tokenizer.pad_token is None:
            # fallback: use [SEP] or eos if needed
        ...
        # Event type tokens (as plain tokens, not special in tokenizer sense)
        # Added "procedureevent" and "emarevent"
        self.event_types = [
            "labevent",
            "inputevent",
            "outputevent",
        ...
        # Save tokenizer to processed_data so training/eval can load it
        os.makedirs("processed_data", exist_ok=True)
        self.tokenizer.save_pretrained("processed_data")
    ...
    def load_events_chunked(
        ...
        for chunk in pd.read_csv(path, usecols=cols, chunksize=chunksize):
            ...
            # Optional item filter (for labs / procedure top-k)
    ...
    def get_time_tokens(self, dt, intime):
        # Compute relative time since ICU admission
        delta = dt - intime
        ...
        # Aggregate minutes into 10-min bins
    ...
    def process_data(self):
        """
        Load ICU stays, events (lab / input / output / procedure / emar),
        join demographics, and keep only events in the first 72h of ICU stay.
        Returns:
            all_data: list of dict(stay_id, demographics, events, intime)
            item_map: dict from itemid (or medication string) to text label
        """
        # 1. ICU stays
        print("Loading ICU stays...")
        ...
        #   These are ICU procedures with numeric values (e.g. settings / measurements)
        # ---------------------------------------------------------------
        proc_cols = [
            "subject_id",
            "hadm_id",
        ...
        # ---------------------------------------------------------------
        # 6. EMAR + EMAR_DETAIL: medication administration from hosp
        # ---------------------------------------------------------------
        print("Loading EMAR + EMAR_DETAIL...")

        # 6.1 emar.csv.gz (no dose columns here)
        ...
        # 6.2 emar_detail.csv.gz for dose_given, dose_given_unit
        ...
        if os.path.exists(detail_path) and not emar_df.empty:
            ...
            # merge dose info back into emar_df
        ...
        # 6.3 normalize EMAR to common event schema
        if not emar_df.empty:
            ...
            # Use medication string as "itemid" so we can reuse create_sequences
            emar_df["medication"] = emar_df["medication"].astype(str)
            emar_df["itemid"] = emar_df["medication"]
        else:
            # keep an empty frame with right columns if nothing left
        ...
        # ---------------------------------------------------------------
        # 7. Item dictionaries for lab / input / output / procedures
        #    (EMAR medications handled separately)
        # ---------------------------------------------------------------
        print("Loading item dictionary tables...")
        d_lab = pd.read_csv(os.path.join(self.data_dir, "d_labitems.csv.gz"))
        d_items = pd.read_csv(os.path.join(self.data_dir, "d_items.csv.gz"))

        item_map = {}
        # lab dictionary
        for _, row in d_lab.iterrows():
            item_map[row["itemid"]] = str(row["label"]).lower()
        # icu dictionary (input / output / procedures)
        for _, row in d_items.iterrows():
            item_map[row["itemid"]] = str(row["label"]).lower()

        # EMAR medication names: add them into item_map using the medication string as key
        if not emar_df.empty:
        ...
        # ---------------------------------------------------------------
        # 8. Demographics: gender / age / race
        # ---------------------------------------------------------------
        ...
        for _, stay in tqdm(
            ...
            # Input events (grouped by stay_id)
            ...
            # Output events (grouped by stay_id)
            ...
            # Procedure events (grouped by stay_id, top-20 itemid only)
            ...
            # EMAR events (grouped by hadm_id)
        ...
        # Optional: inspect a few stays to sanity-check the text format
        if self.inspect:
    ...
    def inspect_events(self, data, item_map):
        ...
        for i in range(n_stays):
            ...
            for _, ev in sub_events.iterrows():
                ...
                # item_name: emar uses "medication", others use itemid -> item_map
                if e_type == "emarevent":
    ...
    def create_sequences(self, data, item_map, split_name):
        ...
        for item in tqdm(data):
            ...
            for _, ev in events.iterrows():
                ...
                # Split content into item_name and (value + unit + [EOE])
                ...
                if len(current_tokens) + len(event_full_tokens) > self.max_len:
                    sequences.append(
                        {"stay_id": stay_id, "input_ids": current_tokens, "type_ids": current_type_ids}
                    )
                    current_tokens = demo_tokens.copy()
        ...
        if eval_items and split_name in ["val", "test"]:
            with open(os.path.join("processed_data", f"{split_name}_eval.pkl"), "wb") as f:
                pickle.dump(eval_items, f)

    def _write_shard(self, data, split_name, shard_idx):
        filename = os.path.join("processed_data", f"{split_name}_shard_{shard_idx}.pkl")
    ...
    def _merge_shards(self, split_name, num_shards):
        ...
        for i in range(num_shards):
            filename = os.path.join("processed_data", f"{split_name}_shard_{i}.pkl")
            if os.path.exists(filename):
                with open(filename, "rb") as f:
                    shard_data = pickle.load(f)
                    all_data.extend(shard_data)
                os.remove(filename)
        with open(os.path.join("processed_data", f"{split_name}.pkl"), "wb") as f:
    ...
    def run(self):
        ...
        train_subs, test_subs = train_test_split(subjects, test_size=0.2, random_state=42)
        val_subs, test_subs = train_test_split(test_subs, test_size=0.5, random_state=42)
        ...
        print(
            f"Train stays: {len(train_data)}, Val stays: {len(val_data)}, Test stays: {len(test_data)}"
        )
...
if __name__ == "__main__":
    ...
    preprocessor = LabTOPPreprocessor(
        data_dir=data_dir,
        stay_limit=None,        # e.g. first 100 stays
        shard_size=5000,
        inspect=True,          # turn on textual inspect
        inspect_max_stays=10,  # print 3 stays
        inspect_max_events=10,  # each 5 events
    )
    preprocessor.run()